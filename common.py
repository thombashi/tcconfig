...
def set_tc(subproc_wrapper, device, rate_mbps, delay_ms, loss_percent):
    import delete_tc

    delete_tc.delete_tc(subproc_wrapper, device)

    command = "tc qdisc add dev %s root handle 1: htb default 0" % (device)
    if subproc_wrapper.run(command) != 0:
        return

    if thutils.common.is_float(rate_mbps):
        command = "tc class add dev %s parent 1:0 classid 1:0 htb rate %dmbit" % (
            device, rate_mbps)
        subproc_wrapper.run(command)
